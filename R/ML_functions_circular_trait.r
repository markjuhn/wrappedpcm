...
#   else {fun=function(X){return(-LogLik_bounds(tree_formatted=tree_formatted,dCoeff=X[1],dMat=DiffMat_backwards_circular_trait(X[2]*SEQ^4+X[3]*SEQ^2+X[4]*SEQ),bounds=bounds))}}
...
lnL_BBMV_circular_trait=function(tree,trait,bounds,a=NULL,b=NULL,c=NULL,Npts){
  ...
  SEQ=seq(from=-1.5,to=1.5,length.out=(Npts+1))[-c(Npts+1)]
  tree_formatted= FormatTree_bounds_circular_trait(tree,trait,V=rep(0,Npts),bounds=bounds)
  ncoeff=(is.null(a)==T)+(is.null(b)==T)+(is.null(c)==T)
  if (is.null(a)==F){
    if (is.null(b)==F){
      # all three shape parameters fixed (e.g. flat landscape if a=b=c=0)
...
find.mle_FPK_circular_trait=function(model,method='Nelder-Mead',init.optim=NULL,safe=F){
  ...
  dMat=DiffMat_backwards_circular_trait(V)
  ll_root=as.data.frame(matrix(NA, Npts,2))
  colnames(ll_root)=c('root','density')
  ll_root$root =seq(bounds[1],to=bounds[2], length.out= (Npts+1))[-c(Npts+1)]
  tree_formatted= FormatTree_bounds_circular_trait(tree,trait,V=rep(0, Npts),bounds=bounds)
